 ...
 *
 * @see https://github.com/JetBrains/intellij-community/blob/master/platform/script-debugger/debugger-ui/src/Variables.kt
  ...
  override fun compare(lhs: String, rhs: String): Int {
    ...
    while (i < string1Length && j < string2Length) {
      ...
      if ((isDecimalDigit(ch1) || ch1 == ' ') && (isDecimalDigit(ch2) || ch2 == ' ')) {
        ...
        while (i < string1Length && isDecimalDigit(lhs[i])) {
          i++
        }
        while (j < string2Length && isDecimalDigit(rhs[j])) {
          j++
        }
