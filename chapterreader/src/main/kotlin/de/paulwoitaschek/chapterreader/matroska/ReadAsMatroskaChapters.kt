...
internal class ReadAsMatroskaChapters @Inject constructor() {
  ...
  private fun isValidEbmlHeader(element: Element): Boolean = if (element isType MatroskaDocTypes.EBML) {
    element.forEachChild {
      if (it isType MatroskaDocTypes.DocType) {
        val docType = it.readString()
        if (docType != "matroska" && docType != "webm") {
          Logger.w("DocType $docType is not de.ph1b.audiobook.chapterreader.de.ph1b.audiobook.chapterreader.matroska")
          return false
        }
      }
    }
    ...
    Logger.w("EBML Header not the first element in the file")
    false
  }
